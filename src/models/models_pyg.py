...
# Assuming get_norm_adj is from your data_utils; include or import as needed
from src.utils.pyg_utils import get_norm_adj

# Custom Convolution Layers (unchanged, as they are specialized)
...
# Factory function for conv layers (unchanged)
...
class GNN(torch.nn.Module):
    """
    Optimized GNN model with configurable parameters.
    - Supports JK via 'jumping_knowledge' (now aliased to --jk arg).
    - Added optional skip_connection for residual links between layers.
    """
    def __init__(
        self,
        num_features,
        ...
        dropout=0,
        conv_type="dir-gcn",
        jumping_knowledge=None,  # None, 'cat', 'max', 'lstm'
        normalize=False,
        alpha=1 / 2,
        learn_alpha=False,
        skip_connection=False,  # New: Enable skip connections
    ):
        ...
        if num_layers == 1:
            self.convs.append(get_conv(conv_type, num_features, output_dim, self.alpha))
        else:
            self.convs.append(get_conv(conv_type, num_features, hidden_dim, self.alpha))
