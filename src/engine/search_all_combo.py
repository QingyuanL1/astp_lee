...
def _has_nonempty_split_file(data_dir: Path, split_name: str) -> bool:
    p = data_dir / f"{split_name}.txt"
    if not p.exists():
        return False
    try:
        with open(p, "r") as f:
...
def _is_valid_train_dataset_dir(data_dir: Path) -> bool:
    if not data_dir.exists():
        return False
    if not (data_dir / "scalers.pkl").exists():
        return False
    if not (data_dir / "templates").exists():
        return False
    if not _has_nonempty_split_file(data_dir, "train"):
        return False
    if not _has_nonempty_split_file(data_dir, "val"):
        return False
    return True


def _resolve_train_data_dir(args: Any, train_size: int) -> Path:
    root_dir = Path(__file__).resolve().parents[2]
    if getattr(args, "data_dir", None):
        cand = Path(str(getattr(args, "data_dir")))
        if _is_valid_train_dataset_dir(cand):
            return cand

    candidates = [
        root_dir / "saved_dataset" / f"ATSP_2560x{train_size}",
        root_dir / "saved_dataset" / f"ATSP_30x{train_size}",
    ]
    ...
    raise FileNotFoundError(
        "No valid training dataset_dir found. "
        "Provide --data_dir pointing to a dataset folder that contains non-empty train.txt/val.txt, "
        "plus scalers.pkl and templates/. "
...
def test_size_500_compatibility(args: Any, relations: Tuple[str, ...], agg: str, params: Dict, framework: str) -> bool:
    """
    Quick test if configuration works on size 250 using single instance.
    Returns True if no OOM, False if OOM.
    Note: Using size 250 instead of 500 since ATSP_30x500 dataset is not available.
    """
    test_args = deepcopy(args)
    test_args.relation_types = list(relations)
    test_args.agg = agg
    test_args.atsp_size = 250
    # Use absolute path to saved_dataset/ATSP_30x250 relative to project root
    root_dir = Path(__file__).resolve().parents[2]
    test_args.data_path = str(root_dir / 'saved_dataset' / 'ATSP_30x250')
    ...
    try:
        ...
        import traceback
        logger.warning(f"Size 250 test error: {str(e)[:200]}")
...
def save_current_best_model(args: Any, relations: Tuple[str, ...], agg: str, params: Dict, 
    ...
    train_size = int(getattr(args, "atsp_size", 50))
    final_args.atsp_size = train_size
    final_args.data_dir = str(_resolve_train_data_dir(final_args, train_size=train_size))
...
def create_objective_with_size_screening(args: Any, rel_subset: Tuple[str, ...], agg_choice: str, 
    ...
    """
    Create Optuna objective with size screening.
    Flow: Sample params -> Screening test -> If OK, train on configured --atsp_size -> Save if best so far
    """
    ...
    def objective(trial: optuna.trial.Trial) -> float:
        ...
        # Ensure hidden_dim >= 16 and divisible by num_heads to avoid dimension mismatch
        ...
        if hidden_dim % num_heads != 0:
            num_heads = min(h for h in [2, 4, 8] if hidden_dim % h == 0)
        ...
        train_size = int(getattr(args, "atsp_size", 50))
        logger.info(
            f"Trial {trial.number}: PASSED size 500 test - proceeding to training on size {train_size}"
        )
        
        # STEP 2: If size 500 works, train on configured size
        train_args = deepcopy(args)
        train_args.relation_types = list(rel_subset)
        train_args.agg = agg_choice
        train_args.atsp_size = train_size
        for k, v in params.items():
        ...
        try:
            ...
            train_args.data_dir = str(_resolve_train_data_dir(train_args, train_size=train_size))
            ...
            logger.warning(f"Trial {trial.number}: OOM during training on size {train_size}")
...
def run_optuna_search(args: Any, n_trials: int = 20, n_jobs: int = 1):
    ...
    agg_methods = ["sum"]  # HetroGAT supports: sum, concat, attn

    train_size = int(getattr(args, "atsp_size", 50))
    logger.info(f"Starting search: {len(relation_subsets)} relation subsets × {len(agg_methods)} agg methods")
    logger.info(f"Each trial: Screening test -> Train on size {train_size} if passed -> Save if best")
    ...
    with open(summary_file, "w") as f:
        ...
        f.write("=" * 50 + "\n")
        f.write(f"Search strategy: Screening test + Size {train_size} training\n")
