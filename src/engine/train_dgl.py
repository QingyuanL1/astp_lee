...
class ATSPTrainerDGL:
    ...
    def train(self, model, trial_id: int = 0, return_best_state: bool = False):
        """Main training loop.

        Args:
            model: torch model to optimize.
            trial_id: trial index (affects logging paths when saving).
            return_best_state: when True, also return a CPU copy of the best
                model.state_dict() observed during training.

        Returns:
            results dict (and optionally the best_state_dict when requested).
        """
