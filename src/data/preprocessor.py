import os
os.environ.setdefault("DGL_NO_GRAPHBOLT", "1")  # avoid optional native GraphBolt import issues
import argparse
import pathlib
import random
        ...
        for size in atsp_sizes:
            ...
            for r in range(1, len(relation_types) + 1):
                for rel_combo in combinations(relation_types, r):
                    ...
                    if save_pyg:
                        if not HAS_PYG:
                            print(f"Skipping PyG template {size}, {rel_str}: torch_geometric not installed")
                        else:
                            try:
